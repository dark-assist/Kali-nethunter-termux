...
function set_strings() {
    ...
    if [[ ${SYS_ARCH} == "arm64" ]];
    then
        echo "[1] NetHunter ARM64 (full. size 2gb=<)"
        echo "[2] NetHunter ARM64 (minimal. size 150mb=<)"
        echo "[3] NetHunter ARM64 (nano. size 200mb=<)"
    ...
    then
        echo "[1] NetHunter ARMhf (full. size 2gb=<)"
        echo "[2] NetHunter ARMhf (minimal. size 140mb=<)"
        echo "[3] NetHunter ARMhf (nano. size 180mb=<)"
        read -p "Enter the image you want to install: " wimg
        if [[ "$wimg" == "1" ]]; then
            wimg="full"
        elif [[ "$wimg" == "2" ]]; then
